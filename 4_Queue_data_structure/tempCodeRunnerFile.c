...
void push_to_stack_one(int x){
    ...
    if(top_one == N -1){
        printf("Stack overflow! The Queue is full.");
    }
    else{
        top_one++;
        stack_one[top_one] = x;
...
void enqueue(int x){
    if(count == N){
        printf("\nQueue is full! Cannot enqueue %d.", x);
    }
    else{
        push_to_stack_one(x);
        count++;
...
void peek(){
    ...
    if(top_two != -1){
        printf("\nCurrent first element of this Queue following FIFO principle is: %d", stack_two[top_two]);
    }
    else{
        for(int i = top_one; i >= 0; i--){
            pop_one_storage = pop_stack_one();
        ...
        printf("\nCurrent first element of this Queue following FIFO principle is: %d", pop_two_storage);

        while(top_two != -1){
            pop_two_storage = pop_stack_two();
            push_to_stack_one(pop_one_storage);
        }
